...
func EncodeJSONResponse(i interface{}, status *int, w http.ResponseWriter, header ...map[string]string) error {
	wHeader := w.Header()
	...
	if len(header) > 0 {
		if len(header) > 1 {
			slog.Warn("Multiple headers provided, only the first will be used", "count", len(header))
		}
